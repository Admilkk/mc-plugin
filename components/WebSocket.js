...
class WebSocketManager {
  ...
  _startLocalServer(config) {
    const {
      mc_qq_ws_port: wsPort,
      mc_qq_ws_url: wsPath,
      mc_qq_ws_password: wsPassword,
    ...
    wss.on("connection", (ws, request) => {
      ...
      try {
        ...
        logger.error(
          WS_LOG_PREFIX +
            `处理来自 ${remoteName ? logger.red(remoteName) : "未知服务器"} 的新连接时出错: ${error.message}`,
        );
    ...
    remoteServers.forEach((serverCfg) => {
      if (serverCfg.ws_able && serverCfg.ws_url && serverCfg.server_name) {
        if (this.activeSockets[serverCfg.server_name]) {
          logger.info(
            WS_LOG_PREFIX + `已存在到 ${serverCfg.server_name} 的连接，跳过`,
          );
        } else {
        ...
        logger.warn(
          WS_LOG_PREFIX +
            `远程服务器配置 ${serverCfg.server_name || "未命名"} 不完整，跳过`,
        );
      }
    });
  }
  ...
  async _establishClientConnection(serverCfg, globalDebug, retries = 0) {
    ...
    logger.info(
      WS_LOG_PREFIX +
        `尝试连接到 ${serverName} (${serverUrl})... (尝试次数: ${retries + 1})`,
    );

    const headers = {
      "X-Self-Name": encodeURIComponent(serverName),
      Authorization: serverToken
        ? `Bearer ${encodeURIComponent(serverToken)}`
        : undefined,
    };
    ...
    ws.on("close", (code, reason) => {
      ...
      if (retries < maxRetries) {
        logger.info(
          WS_LOG_PREFIX +
            `${serverName} 将在5秒后尝试重新连接... (剩余尝试: ${maxRetries - retries})`,
        );
        setTimeout(() => {
    ...
    ws.on("error", (error) => {
      logger.error(
        WS_LOG_PREFIX + logger.red(serverName) + ` 连接错误 ${error.message}`,
      );
      if (
        ...
        if (this.activeSockets[serverName]) {
          ...
          if (retries < maxRetries) {
            logger.info(
              WS_LOG_PREFIX + `${serverName} 因错误将尝试重新连接...`,
            );
            setTimeout(() => {
              this._establishClientConnection(
                serverCfg,
                globalDebug,
                retries + 1,
              );
