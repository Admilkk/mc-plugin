...
class RconManager {
    ...
    try {
      ...
      rconServerList.forEach((serverCfg) => {
        if (
          ...
          logger.warn(
            RCON_LOG_PREFIX +
              `RCON服务器配置 ${serverCfg.server_name || "未命名"} 不完整，跳过`,
          );
        }
      });
    } catch (error) {
  ...
  async _establishConnection(serverCfg, retries = 0) {
    const {
      ...
      rcon_max_attempts: maxRetries = 3,
    } = serverCfg;

    logger.info(
      RCON_LOG_PREFIX +
        `尝试连接到 ${serverName} (${host}:${port})... (尝试次数: ${retries + 1})`,
    );
    ...
    try {
      ...
      rcon.on("end", () => {
        logger.mark(
          RCON_LOG_PREFIX + logger.yellow(serverName) + " RCON连接已断开",
        );
      ...
      if (retries < maxRetries) {
        ...
        setTimeout(
          () => this._establishConnection(serverCfg, retries + 1),
          5000,
        );
      } else {
        logger.error(
          RCON_LOG_PREFIX +
            logger.red(serverName) +
            ` RCON连接失败，已达到最大重连次数 (${maxRetries + 1})，请检查RCON服务是否正常运行`,
        );
      }
    }
  }
