...
const formatMsg = (jsonData, showServer) => {
  ...
  if (handler) {
    ...
    logger.info(
      `服务器 ${jsonData.server_name} 的未知的上报类型 ${jsonData.sub_type}`,
    );
    return null;
  }
};
...
export default function processAndSend(rawMessage) {
  const {
    mc_qq_display_server_name: showServer,
    mc_qq_server_list: serverList,
  } = Config.getConfig();
  let jsonData;
  ...
  const serverCfg = serverList.find(
    (s) => s.server_name === jsonData.server_name,
  );
  ...
  if (serverCfg) {
    ...
    if (!botIds || botIds.length === 0) {
      logger.error(
        `服务器 ${jsonData.server_name} 未配置推送机器人ID，忽略消息`,
      );
      return;
    }
    ...
    botIds.forEach((botId) =>
      groupIds.forEach((groupId) => {
        try {
          ...
          const [, url] =
            messageContent.match(/\[\[CICode,.*?url=([^,$$]+).*?\]\]/) || [];
          console.log(url);
          const msg = url
            ? [
                messageContent.replace(/\[\[CICode,.*?\]\]/, "").trim(),
                segment.image(url),
              ]
            : messageContent;
          Bot[botId].pickGroup(groupId).sendMsg(msg);
          logger.info(
            `机器人 ${botId} 发送到群 ${groupId}${url ? " (含图片)" : ""}`,
          );
        } catch (e) {
          logger.error(
            `机器人 ${botId} 发送到群 ${groupId} 失败: ${e.message}`,
          );
        }
      }),
    );
