...
export class Setting extends plugin {
  ...
  async setting(e) {
    ...
    const index = config.mc_qq_server_list.findIndex(
    ...
    if (isEnable) {
      server.group_list = [
        ...new Set([...(server.group_list || []), e.group_id.toString()]),
      ];
      server.bot_self_id = [
        ...new Set([...(server.bot_self_id || []), e.self_id.toString()]),
      ];
      server.rcon_able = true;
      await e.reply(`✅ 已开启与 ${server_name} 的同步`);
    } else {
      server.group_list = (server.group_list || []).filter(
        (g) => g !== e.group_id.toString(),
      );
      server.bot_self_id = (server.bot_self_id || []).filter(
        (id) => id !== e.self_id.toString(),
      );
